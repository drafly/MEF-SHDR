...
class TRACER_Encoder(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.model = EfficientNet.from_pretrained(f'efficientnet-b{cfg.arch}', weights_path="/home/hg/.cache/torch/hub/checkpoints/adv-efficientnet-b0-b64d5a18.pth", advprop=True, arch=cfg.arch)
...
class EfficientNet(nn.Module):
    def __init__(self, blocks_args=None, global_params=None, arch='0'):
        super().__init__()
    ...
    @classmethod
    def from_name(cls, model_name, in_channels=3, arch='0', **override_params):
        ...
        model = cls(blocks_args, global_params, arch=arch)
        model._change_in_channels(in_channels)
        return model

    @classmethod
    def from_pretrained(cls, model_name, weights_path=None, advprop=False,
                        in_channels=3, num_classes=1000, arch='0', **override_params):
        """create an efficientnet model according to name.
        ...
        """
        model = cls.from_name(model_name, num_classes=num_classes, arch=arch, **override_params)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_channel', type=int, default=3, help='Backbone Architecture')
    parser.add_argument('--pixelShuffleRatio', type=int, default=2, help='Backbone Architecture')

    cfg = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"

    net = Generator(cfg).to(device)
