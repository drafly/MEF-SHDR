...
def compare_ssim(pairs):
    real, fake = pairs
    return structural_similarity(real, fake, channel_axis=2)

def compare_mse(pairs):
    real, fake = pairs
...
def mse(reals, fakes, num_worker=8):
    error = 0
    pool = Pool(num_worker)
    for val in tqdm(pool.imap_unordered(compare_mse, zip(reals, fakes)), total=len(reals), desc='compare_mse'):
        error += val 
    return error / len(reals)
